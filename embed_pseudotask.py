...
class Task2Vec:
    ...
    def __init__(self, model: ProbeNetwork, teacher_model_str, skip_layers=0, max_samples=None, classifier_opts=None,
        ...
        print(classifier_opts, method_opts, loader_opts)
        ...
        # Before teacher string was something like 'resnet18_imagenet'
        # Now allowing addition of XXX_YYY_randomhead
        # If this is the case then want architecture of XXX and pretraining of YYY, but with random head
        # This is call of get_model(XXX, YYY)
        if 'randomhead' in teacher_model_str:
            arch, pretraining, _ = teacher_model_str.split('_')
            self.teacher_model = models.get_model(arch, pretraining).cuda()
        else:
    ...
    def embed(self, dataset: Dataset, reset_classifier=True):
        ...
        if reset_classifier: self.model.reset_classifier()
    ...
    def montecarlo_fisher(self, dataset: Dataset, epochs: int = 1):
        ...
        for k in range(epochs):
            ...
            for i, (data, _) in enumerate(tqdm(data_loader, leave=False, desc="Computing Fisher")):
                data = data.to(device)
                ...
                target = torch.eye(output.size(-1), device=output.device)[torch.multinomial(F.softmax(output, dim=-1), 1).detach().view(-1)]
    ...
    def _run_epoch(self, data_loader: DataLoader, model: ProbeNetwork, loss_fn,
        ...
        for i, (input, _) in enumerate(tqdm(data_loader, leave=False, desc="Computing Fisher")):
            input = input.to(device)
            output = model(input, start_from=skip_layers).log_softmax(dim=1)
            target = self.teacher_model(input).softmax(dim=1)
    ...
    def variational_fisher(self, dataset: Dataset, epochs=1, var_epochs=10, beta=1e-7):
    ...
    def _cache_features(self, dataset: Dataset, indexes=(-1,), max_samples=None, loader_opts: dict = None):
        ...
        for i, (input, _) in tqdm(enumerate(itertools.islice(data_loader, 0, n_batches)), total=n_batches,
                                       leave=False,
                                       desc="Caching features"):
            input = input.cuda()
            output = self.model(input)
            targets.append(self.teacher_model(input).softmax(dim=1).data)
        for hook in hooks:
            hook.remove()
        for index in indexes:
    ...
    def _fit_classifier(self, optimizer='adam', learning_rate=0.0004, weight_decay=0.0001,
        ...
        loss_fn = nn.KLDivLoss(reduction='batchmean') # nn.BCEWithLogitsLoss() # nn.CrossEntropyLoss()
        for epoch in tqdm(range(epochs), desc="Fitting classifier", leave=False):
            metrics = AverageMeter()
            for data, target in data_loader:
                optimizer.zero_grad()
                output = self.model.classifier(data).log_softmax(dim=1)
                loss = loss_fn(output, target)
                error = get_error(output, target)
                loss.backward()
                optimizer.step()
