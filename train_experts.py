...
from sup_training import train_model
...
def main(cfg: DictConfig):
    ...
    expert = train_model(base_network, train_dataset)
    meta = OmegaConf.to_container(cfg, resolve=True)
    meta['task_name'] = getattr(train_dataset, 'task_name', None)
    pickle.dump(meta, open('meta.p', 'wb'))
    torch.save(expert.state_dict(), 'expert.pth')
