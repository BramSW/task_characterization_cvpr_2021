...
class ClassificationTaskExpander(TaskExpander):
    ...
    def __call__(self, task, task_map, label_names=None, flatten_binary=False,
                 ...
                 transform=None, target_transform=None, random_label=''):
        """Call function.
        ...
        random_label : str (default '')
            Whether to use fake random labels
        Returns
        -------
        ...
        new_labels = randomize_labels(new_labels, random_label)
        if random_label: label_names = None
...
def randomize_labels(label_list, random_label):
    """Shuffle the label list according to the random labeling method (called random label)

    Parameters
    ----------
    label_list: list of ints
        list of labels
    random_labeling_method : str
        How to randomize labels:
            '' = no randomization
            'shuffle' = randomly permute current labels
            'linear_rule_k' = k classes per 100 images
            'log_rule_k' = k * log(#images) classes
            'flat_k' = k classes
    """
    if random_label=='shuffle':
        # Just shuffle existing labels for dataset
        new_label_list = deepcopy(label_list)
        random.shuffle(new_label_list)
    elif 'linear_rule' in random_label:
        # String should be in form of 'linear_rule_k' where k is classes per 100 images
        classes_per_100_images = random_label.split('linear_rule_')[1]
        assert classes_per_100_images.isdigit()
        classes_per_100_images = int(classes_per_100_images)
        num_images = len(label_list)
        num_classes = round( classes_per_100_images * num_images / 100)
        new_label_list = [random.randint(0, num_classes-1) for _ in range(num_images)]
    elif 'log_rule' in random_label:
        # Should be of form log_rule_k where there will be k*log(n) classes
        k = random_label.split('log_rule_')[1]
        assert k.isdigit()
        k = int(k)
        num_images = len(label_list)
        num_classes = round( k * np.log(num_images) )
        new_label_list = [random.randint(0, num_classes-1) for _ in range(num_images)]
    elif 'flat' in random_label:
        # should be of form flat_k where there will be k classes per dataset
        k = random_label.split('flat_')[1]
        assert k.isdigit()
        k = int(k)
        num_classes = k
        num_images = len(label_list)
