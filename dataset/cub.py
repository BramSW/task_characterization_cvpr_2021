...
class CUBTasks:
    ...
    def generate(self, task='order', task_id=None, taxonomy_file=TAXONOMY_FILE,
                 ...
                 random_label='', attribute_to_classify=''):
        ...
        attribute_to_classify : str (default '')
            classify on attribute e.g. wing_color instead of class
        Returns
        -------
        ...
        if task_id is not None:
            ...
            if attribute_to_classify:
                from . import attribute_loader
                num_attribute_values = len(attribute_loader.attr_family_to_attr_indices(attribute_to_classify))
                task_map[0]['class_names'] = list(range(num_attribute_values))
                task_map[0]['class_ids'] = task_map[0]['class_names']
                task_map[0]['label_map'] = {100*task_map[0]['task_id']+label:label for label in task_map[0]['class_names']}
            ...
            if random_label: raise NotImplementedError # Not sure how this path works
...
class CUBDataset(ClassificationTaskDataset):
    ...
    def __init__(self, root, split='train', classes_file=CLASSES_FILE,
        ...
        if attribute_to_classify:
            ...
            for i in range(1, 11789):
                if i not in ignore_indices:
                    attr = attribute_loader.attr_family_and_image_index_to_attr_label(attribute_to_classify, i)
                    if attr is None:
                        ignore_indices.add(i)
                    else:
                        labels_list.append(attr)
            label_names = {label:label for label in set(labels_list)}
        else:
        ...
        # read in images
        # Moved this below labels block so can update ignore_indices if attr is None
        images_list = []
        with open(images_file, 'r') as f:
            image_path_to_index = {}
            for l in f:
                index, img = l.strip().split(' ')
                image_path_to_index[img] = index
                if int(index) not in ignore_indices:
