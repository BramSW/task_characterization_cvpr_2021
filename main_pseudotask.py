...
from embed_pseudotask import Task2Vec
from omegaconf import DictConfig, OmegaConf
from copy import deepcopy
import torch
import os

pretraining_to_num_classes = {'imagenet':1000, 'random':1000, 'places365':365}

@hydra.main(config_path="conf/config_pseudotask.yaml")
def main(cfg: DictConfig):
    print(os.listdir('.'))
    if os.path.exists('embedding.p'): asdf
    model_str = '{}_{}'.format(cfg.model.arch, cfg.model.pretraining)
    probe_network = get_model(cfg.model.arch, cfg.model.pretraining, num_classes=pretraining_to_num_classes[cfg.model.pretraining])
    probe_network.cuda()
