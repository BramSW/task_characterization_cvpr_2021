...
sim_matrix = np.load('npy/ttk_cars.npy')
np.fill_diagonal(sim_matrix, -1e6)
names = pickle.load(open('pickles/names.pickle', 'rb'))
error_mat = np.load('npy/car_errs.npy')
errors = { target_name:{source_name:error_mat[j][i] for j,source_name in enumerate(names[:30])}
            for i,target_name in enumerate(names[:30])}
...
spearman_p_values = []
spearman_r_values = []
for name_i1, (name, row) in enumerate(zip(names[:30], sim_matrix)):
    max_sim_i = np.argmax(row)
    match_name = names[max_sim_i]
    rel_errors.append( (errors[name][match_name]- errors[name]['optimal'])/(errors[name]['optimal']+1))
